...
import { getUserAddresses, updateUserAddress, deleteUserAddress } from '@/utils/api';

export default function EditAddressScreen() {
  const router = useRouter();
  const params = useLocalSearchParams();
  const addressIndex = parseInt(params.index as string, 10);
    ...
    const loadAddress = async () => {
      try {
        ...
        if (addresses[addressIndex]) {
          const addr = addresses[addressIndex];
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await updateUserAddress(addressIndex, {
        label,
    ...
    Alert.alert('Delete Address', 'Are you sure you want to delete this address?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          setSaving(true);
          try {
            await deleteUserAddress(addressIndex);
  ...
  return (
    <ThemedView style={styles.container}>
      <View style={styles.header}>
        ...
        <ThemedText style={styles.headerTitle}>Edit Address</ThemedText>
        <Pressable onPress={handleDelete} style={styles.deleteButton} disabled={saving}>
